...
MiMo 输入候选视频、窗口弹幕、主播名和候选时长。视频以临时 720p H.264/AAC 分析副本 Base64 传输；`MIMO_API_KEY` 只从用户环境变量读取，不写入配置和日志。
...
- `MIMO_API_KEY`：MiMo API Key，生产切片必需。
