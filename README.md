...
- 中间可切换源录播与实际最终成片预览，并有连续弹幕密度图和候选导航。密度图支持定位视频与拖拽入点/出点。
- 右侧审核检查器集中编辑标题、简介、标签、边界、字幕文本/时间和字幕样式；技术错误默认折叠，只显示可执行的恢复建议。字幕修改先保存为审核草稿，再由 Windows 重新烧录。
- “通过并生成成片”会创建 Windows `finalize_segment` 任务。只有 ASR、字幕渲染、上传元数据和 SQLite 入队全部成功，片段才进入待发布状态；预览最终成片后，还需单独点击“允许发布”激活投稿。
- 投稿队列和发布表现属于次级工作区；它们只读 dashboard/SQLite 状态，不会在 Pi 上运行重任务。

审核快捷键：`J/K` 切换候选，空格播放/暂停，`I/O` 以当前播放点设置入点/出点，`Ctrl+Enter` 通过并生成成片。输入框、按钮、选择器或原生视频控件聚焦时不会截获快捷键；I/O 只使用源时间。草稿保留在同一标签页的 sessionStorage，刷新可恢复，关闭标签页不保证保留。未完成任务按 jobId 恢复观察，不会自动重新提交。
