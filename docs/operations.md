...
可在 Pi 本机访问 `http://127.0.0.1:2234/api/health`，不应把它作为公开入口。
...
1. 在左侧选择 UP 主分组和直播场次。“启动切片”将所有符合处理条件的待处理录播加入队列，不受当前选中场次或页面筛选影响；只需处理某一场时先选择它，再点击“处理当前录播”。状态筛选用于快速查看待处理、处理中、失败、已完成和已有保留片段的录播；桌面窄宽度下队列可以收起。
2. 在中间预览源录播，点击候选区间跳转；拖拽密度图入点/出点或用 `I/O` 记录当前播放点，修改只形成草稿。
3. 在右侧“内容”页检查 AI 判断、标题、简介、标签和裁剪区间；“字幕”和“技术详情”只在需要时展开。在“字幕”页可逐行修改识别文本和片段内起止时间，也可新增或删除多余行；点击“保存字幕修改”后再重新烧录。
...
8. `/studio/uploads` 查看完整上传/投稿状态。刷新和唤醒上传节点只走 Worker API；不得为了验收而创建真实投稿。
