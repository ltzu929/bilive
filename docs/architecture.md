...
- `manual-keep` 作为兼容接口保留；新工作台使用异步 finalize。drop/range 仍是轻量状态修改，retry/render/reburn/approve-publish 创建 Windows 动作任务；允许发布不在 Pi 写 SQLite，由 Windows 激活 `staged` 队列行。
