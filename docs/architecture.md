...
- MiMo：云端 `mimo-v2.5`，负责候选视频全模态判断和单段粗剪建议。
...
Worker API -> 预检 MIMO_API_KEY、ASR 缓存、SQLite、Videos
watcher -> 原子领取 -> 弹幕密度候选范围
        -> 候选 mp4 -> MiMo 视频判断
        -> drop 删除候选
        -> keep 校验单段 trim -> Whisper 仅转写 trim 音频
        -> 一次 ffmpeg 粗剪+烧字幕
...
- `MIMO_API_KEY` 缺失：预检拒绝启动 watcher，pending 保留。
